...
//  Date:   03.18.2003
//  Description:
//    This is a class that represent the container of project classes.
//
//  Modification:
...
/////////////////////////////////////////////////////////////////////
//
// projectManager.java : For project object container manipulation
//
...
import util.Util;
 ...
 /**
  *
  * <b>General Description:</b>  projectManager extends PstManager.  This class
  * manages all project objects such as creating, retrieving, saving, and
  * deleting project objects.
  *
  * projectManager can only instantiated through the getInstance() method.
  *
  *
...
public class projectManager extends oct.pst.PstManager

{
    public static final String NAME = "project";     // The orgname of Object project
    public static final String CLASSNAME = "oct.codegen.project";     // The className of Object project
    ...
    /**
     * Construct an empty project object
     * @param user A PstUserAbstractObject.
     */
    private projectManager(PstUserAbstractObject user)
    ...
    /**
     * Instantiates a projectManager object.
        ...
        throws PmpManagerCreationException, PmpInternalException
    {
        if(manager == null)
        {
            manager = new projectManager(PstSystem.getInstance());
        }
        return manager;
    }//End getInstance
     ...
     * @param user The PstUserAbstractObject who is requesting to create a project.
     * @param type The type of project to create.<br>
     * @param projectName The name of the project.
     *
     * @exception PmpObjectCreationException There was an error creating the project.
     * @exception PmpIllegalTypeException The specified compnaytype is invalid.
     * @return The data object that was created.  An explicit cast is required to convert to a project object.
     */
    public PstAbstractObject create(PstUserAbstractObject user, String projectName)
        throws PmpIllegalTypeException, PmpObjectCreationException
    {
        return create(user.getSession(), projectName);
    }//End create
     ...
     * @param projectName The name of the project.
     * @exception PmpObjectCreationException There was an error creating the project.
     * @exception PmpIllegalTypeException The specified project type is invalid.
     * @return The data object that was created.  An explicit cast is required to convert to a project object.
     */
    PstAbstractObject create(OmsSession session, String projectName)
        throws PmpIllegalTypeException, PmpObjectCreationException
    {
        project project = null;
        if(projectName == null || projectName.length() == 0)
        {
            throw new PmpObjectCreationException("The project name cannot be empty.");
        }

        try
        {

            project = new project(session,m_organization, projectName);
            return project;
        }
        catch(PmpException pe)
        {
            throw new PmpObjectCreationException("Error creating project object:" + pe.toString());
        }
    }//End create
    ...
    {
		project newUser = null;
		try
		{
			newUser = new project(user, m_organization, memberName, password);
		}
		catch(PmpException e)
		{
			throw new PmpObjectCreationException("Cannot create new project");
		}
    	...
    	throws PmpObjectCreationException, PmpInternalException
    {
		...
		try
		{
			newUser = new project(user, m_organization);
		}
		catch(PmpException e)
		{
			throw new PmpObjectCreationException("Cannot create new project");
    ...
    /**
     *
     * Obtain a project object from database.
     * @param user  PstUserAbstractObject requesting a project object.
     * @param objectId Object id of the project object.
     ...
     * @return The data object with the specified id.  An explicit cast is required to convert to a project object.
     */
        ...
        throws PmpObjectNotFoundException, PmpInternalException
    {
        try
        {
            ...
            {
                throw new PmpObjectNotFoundException("Member not in project organization.");
            }
    ...
    /**
     *
     * Obtain a project object from the database.
     * @param user  A PstUserAbstractObject.
     * @param objectName Object name of the project object.
     * @exception PmpObjectNotFoundException Could not find the specified object.
     * @exception PmpInternalException An internal error occurred.
     * @return The data object with the specified id.  An explicit cast is required to convert to a project object.
     */
    ...
    /**
     *
     * Obtain an array of project objects based on a set of specified object ids.
     * @param user  PstUserAbstractObject requesting the project objects.
     * @param objectIds Object ids of all project objects to retrieve.
     ...
     *  the data object to a project object.
     *   <b>NOTE:</b>  The arrays of project will be sorted by object ids,
    ...
    /**
     *
     * Obtain a Company object based on a set of project ids.
     * Obtain an array of project objects based on a set of specified object ids.
     * @param session  PstUserAbstractObject requesting the project objects.
     * @param objectIds Object ids of all project objects to retrieve.
     * @exception PmpObjectNotFoundException Could not find the specified object(s).
     * @exception PmpInternalException An internal error occurred.
     * @return An array of data objects with the specified ids.  An explicit cast is required to convert
        ...
        throws PmpObjectNotFoundException, PmpInternalException
    {
        try
        {
            ...
            project [] result = new project[newCount];

            for(int i=0; i<newCount; i++)
            {
                OmsMember member = (OmsMember) memList.get(i);
                //Return a project object
                result[i] = new project(member);
    ...
    /**
     *
     * Obtain an array of project objects based on a set of specified project member names.
     * @param session  PstUserAbstractObject requesting the project objects.
     * @param objectNames Object names of all project objects to retrieve.
     * @exception PmpObjectNotFoundException Could not find the specified object(s).
     * @exception PmpInternalException An internal error occurred.
     * @return An array of data objects with the specified ids.  An explicit cast is required to convert
     *  the data object to a project object.
        ...
        throws PmpObjectNotFoundException, PmpInternalException
    {
        ...
        project [] newResult = new project[newCount];
        for(int i=0; i<newCount; i++)
        {
            newResult[i] = (project) result.get(i);
        }
    ...
    /**
     *
     * Delete the member from the database.  See also <code>util.Util.deleteProject()
     * </code> for a comprehensive delete operation of the project object.
     * @param dataObject The project object to delete permanently.
     ...
     * @param dataObject The project object to save.
     ...
     * @param dataObject The project object to refresh.
     */
    public void refresh(PstAbstractObject dataObject)
    {
        ((project)dataObject).refresh();

    }//End refresh


    /**
     * Obtain a List of project objects based on on the user ID.
     *
     * @param user  PstUserAbstractObject requesting the project objects.
     * @exception PmpObjectNotFoundException Could not find the specified object(s).
     * @exception PmpInternalException An internal error occurred.
     * @return An List of project objects that is own by this user.
     *
     */
    public List getAllproject(PstUserAbstractObject user)
        throws PmpObjectNotFoundException, PmpInternalException
    {
		...
		{
			projectList.add((project)objArray[i]);
		}
    ...
    /**
     * Obtain a List of project objects that belongs to the project.
     *
     * @param projID - int value of a project ID.
     * @param user - PstUserAbstractObject requesting the project objects.
     * @exception PmpObjectNotFoundException Could not find the specified object(s).
     * @exception PmpInternalException An internal error occurred.
     * @return An List of project objects that is own by this user.
     *
     */
    public List getAllproject(PstUserAbstractObject user, int projID)
        throws PmpObjectNotFoundException, PmpInternalException
    {
		List projectList = new ArrayList();
...
*/
    ...
    /**
     * Get all projects that the user is authorized to access.
     * @return An array of all the project ids authorized to be accessed.
     */
    public int [] getProjects(PstUserAbstractObject u)
    	throws PmpException
	{
    	return getProjects(u, true);
	}
    public int [] getProjects(PstUserAbstractObject u, boolean bShowClosedPj)
    	throws PmpException
    {
		int [] ids = new int[0];
		int uid = u.getObjectId();
		int iRole = Util.getRoles(u);
		String s;
		String [] sa;

		// @ECC071906
		String filter = null;
		String townIdS = null;	// (String)u.getAttribute("TownID")[0];
		/*Integer io = (Integer)u.getAttribute("Towns")[0];	// only look at the first one: should be his own company
		if (io != null)
			townIdS = io.toString();*/
		townIdS = u.getStringAttribute("Company");
		...
		{
			if (townIdS != null)
				filter = "Company='" + townIdS +"'";		// view his company (used to be TownID)
			else
				filter = "om_acctname='%'";				// one company only or view all company
			filter = "(" + filter + ") || (TeamMembers=" +uid+ ")";
		}
		else if ( (iRole & user.iROLE_DIRECTOR) > 0 )
		{
			// director
			userinfo ui = (userinfo)userinfoManager.getInstance().get(u, String.valueOf(uid));
			Object [] o = ui.getAttribute("Preference");
			for (int i=0; i<o.length; i++)
			{
				s = (String)o[i];
				if (s.startsWith(userinfo.PREF_PROJECT_ID))
				{
					// ProjectID:12345:12346:12347
					sa = s.split(":");
					ids = new int[sa.length-1];
					for (int j=1; j<sa.length; j++)
						ids[j-1] = Integer.parseInt(sa[j]);
					break;
				}
			}
		}
		...
		{
			// regular user
			filter = "(TeamMembers=" +uid+ ")";
			if (!isMultiCorp) {
				// if !isMultiCorp, see all public project
				filter += "||(Type='Public%')";
			}
			if (isMultiCorp && townIdS!=null)
			{
				filter += " || (TownID='" + townIdS + "' && Type='Public%')";	// view also his company
			}
		}
		...
		if (filter != null)
		{
			filter = "(" + filter + ")";
			if (!bShowClosedPj)
				filter += " && (Status!='" + project.ST_CLOSE + "')";	// don't show closed projects
			//System.out.println("getProjects(): " + filter);
			ids = findId(u, filter);
		}
		return ids;
	}

	public project getProjectByAbbreviation(PstUserAbstractObject u, String abbrev)
		throws PmpException
	{
		if (Util.isNullOrEmptyString(abbrev))
			return null;

		String s;
		int [] ids = getProjects(u, false);
		for (int i=0; i<ids.length; i++) {
			project pj = (project)manager.get(u, ids[i]);
			s = pj.getOption(project.ABBREVIATION);
			if (s == null)
				continue;
